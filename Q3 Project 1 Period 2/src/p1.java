		...
		try {
			...
			for (int i = 0; i < name.length - 1; i++) { // for loop to check the switches
				...
				if (name[i].equals("--Help")) {
					System.out.println(
							"This program is used to guide the Wolverine through a maze to find the Wolverine Buck. ");
					...
					System.out.println(
							"'--Incoordinate' : this switch is turned on to input file is in coordinate format.");
		...
		try {
			...
			for (int r = 0; r < mapArr.length; r++) {
				...
				for (int c = 0; c < cols; c++) {
					...
					if (!(newRow.substring(c, c + 1).equals("w")) && !(newRow.substring(c, c + 1).equals("W"))
	...
	public static void readQueueFile(String fileName) throws IllegalMapCharacterException, IncorrectMapFormatException {
		...
		try {
			...
			while (scanner.hasNext()) {
				...
				if (!value.equals("w") && !value.equals("W") && !value.equals("@") && !value.equals("$")
